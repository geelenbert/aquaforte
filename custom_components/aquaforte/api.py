...
class AquaforteDiscoveryClient:
    ...
    def _parse_response(self, message: bytes, remote: tuple) -> None:
        ...
        if message_type == 0x04:  # DISCOVERY_RESPONSE
...
class AquaforteApiClient:
    ...
    def __init__(self, discovery_data: dict) -> None:
        ...
        self._wifi_version = discovery_data.get('wifi_version')
        self._product_key = discovery_data.get('product_key')
        self._mcu_attributes = discovery_data.get('mcu_attributes')
        self._api_server = discovery_data.get('api_server')
    ...
    async def async_connect_device(self) -> bool:
        """Connect to the specific AquaForte device over TCP."""
        _LOGGER.debug(f"Connecting to device at {self._ip_address}...")
        try:
